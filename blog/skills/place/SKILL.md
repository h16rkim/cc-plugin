...
| link | **권장** | 지도 URL (카카오맵 또는 구글맵) | "https://place.map.kakao.com/133408066" |
...
**link 필드 필수 포함:** 장소를 추가/수정할 때 `link` 필드에 **카카오맵 또는 구글맵 URL을 반드시 포함**합니다. 사용자가 link를 제공하지 않은 경우, `fetch-kakaomap` 또는 `fetch-googlemap` 스킬을 사용하여 해당 장소의 지도 URL을 직접 조회한 후 포함합니다. 국내 장소는 카카오맵(`https://place.map.kakao.com/{PLACE_ID}`), 해외 장소는 구글맵 URL을 우선 사용합니다.
...
### link 필드 필수 포함
- 장소를 추가/수정할 때 `link` 필드에 **카카오맵 또는 구글맵 URL을 반드시 포함**합니다. 사용자가 link를 제공하지 않은 경우, `fetch-kakaomap` 또는 `fetch-googlemap` 스킬을 사용하여 해당 장소의 지도 URL을 직접 조회한 후 포함합니다. 국내 장소는 카카오맵(`https://place.map.kakao.com/{PLACE_ID}`), 해외 장소
